...
func main() {
	...
	var worstGuardID, maxMinute, maxTotal int
	for guardID, byMinute := range asleepByMinute {
		for minute, minuteTotal := range byMinute {
			if maxTotal < minuteTotal {
				worstGuardID = guardID
				maxTotal = minuteTotal
				maxMinute = minute
			}
		}
	}

	log.Context("solution").Printf("guard: %d worst %d", worstGuardID, maxMinute)
}
