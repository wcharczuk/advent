...
func main() {
	...
	for _, element := range elements {
		...
		log.Context("debug").Printf("start: %s => %s (%d)", string(element), string(working), len(working))
		var ok bool
		for {
		...
		log.Context("debug").Printf("end: %s => %s (%d)", string(element), string(working), len(working))
		if len(working) < min {
			min = len(working)
		}
	}
