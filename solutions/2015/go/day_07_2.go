...
func checkWireReference(w wire, wires map[string]wire) bool {
	if typed, isTyped := w.(*direct); isTyped {
		if typed.Input == nil {
			println(typed.Id, "has unwired input:", typed.InputId)
		...
		if typed.A == nil {
			println(typed.Id, "has unwired input:", typed.AId)
		...
		if typed.B == nil {
			println(typed.Id, "has unwired input:", typed.BId)
...
func main() {
	...
	if b, hasB := wires["b"]; hasB {
		if typed, isTyped := b.(*input); isTyped {
			typed.Value = 16076
		}
	}
